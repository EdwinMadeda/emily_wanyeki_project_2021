...
array_push($programmes, $row_set['name']);
}
...
if(isset($_POST["btn_submit"])){
  if($_POST["student_programme"]!="" && $_POST["student_session"]!=""){
    ...
    if($check_prevention==1){
      ...
      if($check_user==1){
       echo "<script>alert('Sorry You Have Registered Already')</script>";
        ...
        $insert_details_command = "INSERT INTO `industrial_registration` (`id`, `first_name`, `last_name`,`other_name`, `programme`, `session`,`faculty`,`date`, `index_number`) VALUES (NULL, '$student_fname', '$student_lname','$other_name', '$student_programme_selected', '$student_session_selected','$student_faculty', CURRENT_TIMESTAMP, '$student_index')";
